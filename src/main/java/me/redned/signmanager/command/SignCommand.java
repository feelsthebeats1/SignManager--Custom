...
public class SignCommand implements TabExecutor {
            ...
            case "admin" -> {
                ...
                switch (adminSubCommand) {
                    ...
                    case "command" -> {
                        ...
                        String commandToRun = String.join(" ", Arrays.copyOfRange(args, 4, args.length));

                        boolean runAsOp;
                        if (runType.equalsIgnoreCase("op")) {
                            runAsOp = true;
                        } else if (runType.equalsIgnoreCase("player")) {
                            runAsOp = false;
                        } else {
                            audience.sendMessage(Component.text("Invalid run type! Valid types: op, player.", NamedTextColor.RED));
                            return true;
                        }
                        ...
                        if (side.equalsIgnoreCase("all")) {
                            this.plugin.setSignCommand(sign, Side.FRONT, commandToRun, runAsOp);
                            this.plugin.setSignCommand(sign, Side.BACK, commandToRun, runAsOp);

                            audience.sendMessage(Component.text("Successfully set command to run on all sides of the sign as " + runType.toLowerCase() + ".", NamedTextColor.GREEN));
                        } else if (side.equalsIgnoreCase("front")) {
                            this.plugin.setSignCommand(sign, Side.FRONT, commandToRun, runAsOp);
                            audience.sendMessage(Component.text("Successfully set command to run on the front side of the sign as " + runType.toLowerCase() + ".", NamedTextColor.GREEN));
                        } else if (side.equalsIgnoreCase("back")) {
                            this.plugin.setSignCommand(sign, Side.BACK, commandToRun, runAsOp);
                            audience.sendMessage(Component.text("Successfully set command to run on the back side of the sign as " + runType.toLowerCase() + ".", NamedTextColor.GREEN));
                        } else {
    ...
    private void sendHelp(Player player, Audience audience) {
        ...
        if (player.hasPermission("signmanager.command.sign.admin")) {
            ...
            audience.sendMessage(Component.text("/sign admin command <front|back|all> <op|player> <command> ", NamedTextColor.GREEN)
        ...
        if (args.length >= 2) {
            ...
            if (args[0].equalsIgnoreCase("admin") && sender.hasPermission("signmanager.command.sign.admin")) {
                ...
                if (args[1].equalsIgnoreCase("command")) {
                    ...
                    if (args.length == 4) {
                        return StringUtil.copyPartialMatches(args[3], List.of("op", "player"), completions);
                    }
                }
            }
        }
